...
class Robot(object):
    def __init__(self, is_sim, obj_mesh_dir, num_obj, workspace_limits,
        ...
        if self.is_sim:
            ...
            self.csv_file = self.obj_mesh_dir + 'objects.csv'            # path to the csv file with list of objects index,name, path
            self.num_obj = num_obj
            

            # Connect to simulator - PyBullet 
            self.env = Env(obj_dir = self.obj_mesh_dir, is_GUI=True, box_size=(0.4,0.4,0.3), resolution = 40)
            
            # Randomly choose objects to add to scene
            self.tot_num_objects = self.env.generate_urdf_csv()
            self.obj_mesh_ind = np.random.randint(0, self.tot_num_objects , size=self.num_obj)

...
def main(args):
    ...
    # ------------- Algorithm options -------------
    method = args.method # 'fisrt_stage' or 'second_stage'
    ...
    # Initialize trainer
    trainer = Trainer(method, rewards, future_reward_discount,
    ...
    # Initialize variables for exploration probability
    explore_prob = 0.5 if not is_testing else 0.0

    # Quick hack for nonlocal memory between threads in Python 2
    ''' nonlocal_variables dictionary is used to share state information between the main thread and the parallel thread (process_actions())'''
    ...
    for i in range(20):
        ...
        if not exit_called:
            ...
            packing_prediction = trainer.forward(color_heightmap, valid_depth_heightmap, is_volatile=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--obj_mesh_dir', dest='obj_mesh_dir', action='store', default='objects/',                        help='directory containing 3D mesh files (.obj) of objects to be added to simulation')
    parser.add_argument('--num_obj', dest='num_obj', type=int, action='store', default=10,                                help='number of objects to add to simulation')
    parser.add_argument('--tcp_host_ip', dest='tcp_host_ip', action='store', default=False,                               help='IP address to robot arm as TCP client (UR5)')
    parser.add_argument('--tcp_port', dest='tcp_port', type=int, action='store', default=None,                            help='port to robot arm as TCP client (UR5)')
    parser.add_argument('--rtc_host_ip', dest='rtc_host_ip', action='store', default=None,                                help='IP address to robot arm as real-time client (UR5)')
    parser.add_argument('--rtc_port', dest='rtc_port', type=int, action='store', default=None,                            help='port to robot arm as real-time client (UR5)')
    ...
    # ------------- Algorithm options -------------
    parser.add_argument('--method', dest='method', action='store', default='stage_1',                                     help='set to \'stage_1\' (pre-train Worker) or \'stage_2\' (train Worker and Manager)')
    parser.add_argument('--rewards', dest='push_rewards', action='store_true', default=False,                             help='use immediate rewards?')
